...
export function createStackedBar<T extends StatMap>(rootDiv: HTMLElement, graphName: string, data: StackedDataPoint<T>[], keys: (Extract<keyof T, string>)[], colors: string[], altName: (s: Extract<keyof T, string>) => string) {
  ...
  groups.each(function (series) {
    ...
    group.selectAll('text.label1')
      ...
      .attr('fill', () => getContrastYIQ(colors[keys.indexOf(series.key)]))
      .text(d => `${Math.round(100 * d.data.stat[series.key as keyof T] / sumStats(d.data.stat))}%`)
