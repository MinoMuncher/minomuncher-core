...
export function createOverflowStackedBar<T extends StatMap>(rootDiv: HTMLElement, graphName: string, data: OverflowDataPoint<T>[], ceiling: number, keys: (Extract<keyof T, string>)[], colors: string[], altName: (s: keyof T) => string, starStep: number) {
